...
namespace DAL
{
    ...
    public int Add(Model.UserInfo user)
    {
        using (IDbConnection cn = new MySqlConnection(cns))
        {
            string sql = "insert into userinfo valuse(@userName,@passWord,@qq,@email,@type,@userImg);";
    ...
    public int Update(Model.UserInfo user)
    {
        using (IDbConnection cn = new MySqlConnection(cns))
        {
            string sql = "update userinfo set set password=@password,qq=@qq,email=@email,type=@type,userimg=@userImg where username=@userName";
            return cn.Execute(sql, user);
        }
    }
